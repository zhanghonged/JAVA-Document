...
![03-if-elseif-else](images/03-if-elseif-else.png)
...
![03-while循环](images/03-while循环.png)
...
`do...while`循环的特点：无条件执行一次循环体，即使我们将循环条件直接写成false，也依然会循环一次。这样的循环具有一定的风险性，因此初学者不建议使用`do...while`循环。
